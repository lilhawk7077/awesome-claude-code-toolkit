...
| [onboard](https://github.com/ApurvBazari/claude-plugins) | Analyze existing codebases and generate tailored Claude tooling — CLAUDE.md, path-scoped rules, skills, agents, hooks, and CI/CD. Plugin-aware generation skips agents whose capabilities are already covered by installed plugins. Includes `/onboard:evolve` for drift detection. MIT |
| [forge](https://github.com/ApurvBazari/claude-plugins) | Greenfield project scaffolder for Claude Code. 4-phase wizard (context → scaffold → AI tooling → lifecycle), stack-agnostic via web search, walking-skeleton mode for architectures without a mature CLI, and session resume via `.claude/forge-state.json`. Delegates Claude tooling generation to companion `onboard` plugin. MIT |
